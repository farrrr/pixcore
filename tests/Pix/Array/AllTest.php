...
class Pix_Array_AllTest_TestObject
{
    private $v;
    protected $__time;
    protected $__id;
...
class Pix_Array_AllTest extends PHPUnit_Framework_TestCase
{
	...
	$this->assertEquals($arr[2], null);
	$this->assertEquals($arr['test'], 'c');
	$this->assertEquals($arr['bar'], null);
	...
	unset($arr[0]);
	$this->assertEquals($arr[0], null);
	$this->assertEquals($arr['test'], 'f');
	unset($arr['test']);
	$this->assertEquals($arr['test'], null);
	unset($arr[0]);
	$this->assertEquals($arr[0], null);
	$this->assertEquals($arr['lala'], null);
	unset($arr['lala']);
	$this->assertEquals($arr['lala'], null);
	...
	$obj = new Pix_Array_AllTest_TestObject();
	$obj->set('lala');
	$arr['test'] = $obj;
	$this->assertEquals(is_object($arr['test']), true);
	$this->assertEquals(get_class($arr['test']), 'Pix_Array_AllTest_TestObject');
	...
	array_shift($order);
	$this->assertEquals($order['value'], null);
	$this->assertEquals($order['name'], 'asc');
	...
	$obj = new Pix_Array_AllTest_TestObject(1321321, 3214);
	$objs[] = $obj;
	$obj = new Pix_Array_AllTest_TestObject("1321", 214);
	$objs[] = $obj;
	$obj = new Pix_Array_AllTest_TestObject(132131, "9123");
	$objs[] = $obj;
	$obj = new Pix_Array_AllTest_TestObject(8921321, 4214);
	$objs[] = $obj;
    ...
    public function testSeek($array)
    {
        ...
        $array = Pix_Array::factory($array);
        while ($i++ < 5) {
            $random = rand(0, count($original_array));
    ...
    public function testGetProperty($array)
    {
        ...
        foreach ($original_array as $item) {
            $this->assertEquals($original_array[$item], $array->{$item});
        }
    }
